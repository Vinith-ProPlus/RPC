				...
				<div class="card-header text-center">
					<div class="row">
						...
						<div class="col-sm-4 my-2 text-right">
							@if($crud['view']==1)
								<a href="{{url('/')}}/admin/master/general/states/" class="btn  btn-outline-dark {{$Theme['button-size']}}" type="button" > Back </a>
							@elseif($crud['add']==1)
								<a href="{{url('/')}}/admin/master/general/states/create" class="btn  btn-outline-dark {{$Theme['button-size']}}" type="button" >Back</a> <!-- full-right -->
							@endif
						</div>
					</div>
				</div>
				<div class="card-body " >
					<div class="row">
						<div class="col-12 col-sm-12 col-lg-12">
							<table class="table {{$Theme['table-size']}}" id="tblStates">
								<thead>
									<tr>
    ...
    $(document).ready(function(){
        ...
        const LoadTable=async()=>{
			...
			$('#tblStates').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url": "{{url('/')}}/admin/master/general/states/trash-data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnRestore',function(){
			...
			swal({
                title: "Are you sure?",
                text: "You want Restore this State!",
                type: "warning",
            ...
            function(){
            	...
            	$.ajax({
            		type:"post",
                    url:"{{url('/')}}/admin/master/general/states/restore/"+ID,
