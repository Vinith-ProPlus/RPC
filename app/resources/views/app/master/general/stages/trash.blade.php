				...
				<div class="card-header text-center">
					<div class="row">
						...
						<div class="col-sm-4 my-2 text-right">
							@if($crud['view']==1)
								<a href="{{url('/')}}/admin/master/general/stages/" class="btn  btn-outline-dark {{$Theme['button-size']}}" type="button" > Back </a>
							@elseif($crud['add']==1)
								<a href="{{url('/')}}/admin/master/general/stages/create" class="btn  btn-outline-dark {{$Theme['button-size']}}" type="button" >Back</a> <!-- full-right -->
							@endif
						</div>
					</div>
				</div>
    ...
    $(document).ready(function(){
        ...
        const LoadTable=async()=>{
			...
			$('#tblStages').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url": "{{url('/')}}/admin/master/general/stages/trash-data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnRestore',function(){
			...
			swal({
                title: "Are you sure?",
                text: "You want Restore this Stage!",
                type: "warning",
            ...
            function(){
            	...
            	$.ajax({
            		type:"post",
                    url:"{{url('/')}}/admin/master/general/stages/restore/"+ID,
