		...
		<div class="row">
			<div class="col-sm-12">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item"><a href="{{url('/')}}/admin/master/general/postal-codes/" data-original-title="" title="">{{$PageTitle}}</a></li>
				...
				<div class="card-body " >
                    <div class="row">
                        <div class="col-sm-12 mt-20">
                            <div class="form-group">
                                <label class="txtPostalCode"> Postal Code <span class="required"> * </span></label>
                                <input type="number" class="form-control  {{$Theme['input-size']}}" id="txtPostalCode" value="<?php if($isEdit==true){ echo $EditData[0]->PostalCode;} ?>">
                                <div class="errors" id="txtPostalCode-err"></div>
                            </div>
                        </div>
                    ...
                    <div class="row mt-20">
                        <div class="col-sm-12 text-right">
                            @if($crud['view']==true)
                            <a href="{{url('/')}}/admin/master/general/postal-codes" class="btn {{$Theme['button-size']}} btn-outline-dark mr-10" id="btnCancel">Back</a>
                            @endif
...
<script>
    $(document).ready(function(){
        ...
        const formValidation=()=>{
            ...
            if(PostalCode==""){
                $('#txtPostalCode-err').html('The Postal Code is required.');status=false;
            }else if(!(PostalCode.length == 6)){
                $('#txtPostalCode-err').html('Postal Code must be 6 digits');status=false; 
            }
            if(CountryID==""){
        ...
        $('#btnSave').click(function(){
            ...
            if(status){
                swal({
                    title: "Are you sure?",
                    text: "You want @if($isEdit==true)Update @else Save @endif this Postal Code!",
                    type: "warning",
                    ...
                    swal.close();
                    btnLoading($('#btnSave'));
                    let postUrl=@if($isEdit==true) "{{url('/')}}/admin/master/general/postal-codes/edit/{{$EditData[0]->PID}}"; @else "{{url('/')}}/admin/master/general/postal-codes/create"; @endif
                        ...
                        success:function(response){
                            ...
                            if(response.status==true){
                                swal({
                                    ...
                                    @if($isEdit==true)
                                        window.location.replace("{{url('/')}}/admin/master/general/postal-codes");
                                    @else
