				...
				<div class="card-header text-center">
					<div class="row">
						...
						<div class="col-sm-4 my-2 text-right text-md-right">
							@if($crud['restore']==1)
								<a href="{{url('/')}}/admin/master/general/construction-service-category/trash" class="btn  btn-outline-dark {{$Theme['button-size']}} m-r-10" type="button" > Trash </a>
							@endif
							@if($crud['add']==1)
								<a href="{{url('/')}}/admin/master/general/construction-service-category/create" class="btn  btn-outline-success btn-air-success {{$Theme['button-size']}}" type="button" >Create</a>
							@endif
						</div>
					</div>
				</div>
					...
					<div class="row">
						<div class="col-12 col-sm-12 col-lg-12">
                            <table class="table {{$Theme['table-size']}}" id="tblConType">
                                <thead>
                                    <tr>
                                        <th>Construction Service Category ID</th>
                                        <th>Construction Service Category Name</th>
    ...
    $(document).ready(function(){
        ...
        const LoadTable=async()=>{
			...
			tblConType=$('#tblConType').dataTable({
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url": "{{url('/')}}/admin/master/general/construction-service-category/data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnEdit',function(){
			window.location.replace("{{url('/')}}/admin/master/general/construction-service-category/edit/"+ $(this).attr('data-id'));
		});
		...
		$(document).on('click','.btnDelete',function(){
			...
			swal({
                title: "Are you sure?",
                text: "You want Delete this Construction Service Category!",
                type: "warning",
            ...
            function(){swal.close();
            	$.ajax({
            		type:"post",
                    url:"{{url('/')}}/admin/master/general/construction-service-category/delete/"+ID,
