    ...
    $(document).ready(function(){
        ...
        const getProductDetails=async()=>{
            ProductDetails=await new Promise((resolve,reject)=>{
                ...
                $.ajax({
                    type:"post",
                    url:"{{url('/')}}/master/product/products/get/product-details/"+ProductID,
        ...
        const getAttributes=async()=>{
            ...
            $.ajax({
                type:"post",
                url:"{{url('/')}}/master/product/products/get/attributes",
                headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                data:{PCID,PSCID},
                dataType:"json",
                async:true,
        ...
        const getAttributeValues=async(AID)=>{
            let result=await new Promise((resolve,reject)=>{
                let PCID=$('#lstCategory').val();
                let PSCID=$('#lstSubCategory').val();
                $.ajax({
                    type:"post",
                    url:"{{url('/')}}/master/product/products/get/attribute-details",
                    headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                    dataType:"json",
                    data:{PCID,PSCID,AID},
                    async:true,
        ...
        const AddAttributes=async(AID,AName)=>{
            let AValues=await getAttributeValues(AID);
                                ...
                                html+='<div class="col-12">';
                                    html+='<div class="form-group">';
                                        ...
                                        for(let AVal of AValues){
                                            html+='<option data-value-text="'+AVal.Values+'" value="'+AVal.ValueID+'">'+AVal.Values+' </option>'
                                        }
                                        html+='</select>';
                                    html+='</div>';
                                html+='</div>';
        ...
        const getCategory=async()=>{
            ...
            $.ajax({
                type:"post",
                url:"{{url('/')}}/master/product/products/get/category",
                headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                dataType:"json",
                async:true,
                error:function(e, x, settings, exception){ajaxErrors(e, x, settings, exception);},
        ...
        const getSubCategory=async()=>{
            ...
            $.ajax({
                type:"post",
                url:"{{url('/')}}/master/product/products/get/sub-category",
                headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                data:{CID},
                dataType:"json",
                async:true,
        ...
        const getTax=async()=>{
            ...
            $.ajax({
                type:"post",
                url:"{{url('/')}}/master/product/products/get/tax",
                headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                dataType:"json",
                async:true,
                error:function(e, x, settings, exception){ajaxErrors(e, x, settings, exception);},
        ...
        const getUOM=async()=>{
            ...
            $.ajax({
                type:"post",
                url:"{{url('/')}}/master/product/products/get/uom",
                headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                dataType:"json",
                async:true,
                error:function(e, x, settings, exception){ajaxErrors(e, x, settings, exception);},
        ...
        const save=async(formData)=>{
            ...
            let postUrl= @if($isEdit) "{{url('/')}}/master/product/products/edit/{{$ProductID}}"; @else "{{url('/')}}/master/product/products/create"; @endif
            ...
            const confirmation=async()=>{
                if(VariationSaveStatus){
                    ...
                    $.ajax({
                        ...
                        beforeSend:async()=>{
                            setTimeout(() => {
                                eventSource = new EventSource("{{url('/')}}/master/product/products/get/process-status");
                        ...
                        success:function(response){
                            if(response.status){
                                swal({
                                    ...
                                    @if($isEdit==true)
                                        window.location.replace("{{url('/')}}/master/product/products");
                                    @else
