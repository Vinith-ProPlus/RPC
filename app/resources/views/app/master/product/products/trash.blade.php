		...
		<div class="row">
			<div class="col-sm-12">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item"><a href="{{ url('/') }}/admin/master/product/products/" data-original-title="" title="">{{$PageTitle}}</a></li>
				...
				<div class="card-header text-center">
					<div class="row">
						...
						<div class="col-sm-4 my-2 text-right text-md-right">
							@if($crud['view']==1)
							<a href="{{ url('/') }}/admin/master/product/products/" class="btn  btn-outline-dark {{$Theme['button-size']}}" type="button" > Back </a>
							@elseif($crud['add']==1)
							<a href="{{ url('/') }}/admin/master/product/products/create" class="btn  btn-outline-dark {{$Theme['button-size']}}" type="button" >Back</a> <!-- full-right -->
							@endif
						</div>
					</div>
				</div>
    ...
    $(document).ready(function(){
        const LoadTable=async()=>{
			...
			$('#tblProducts').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url": "{{url('/')}}/admin/master/product/products/trash-data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnRestore',function(){
            ...
            function(){
            	...
            	$.ajax({
            		type:"post",
                    url:"{{url('/')}}/admin/master/product/products/restore/"+ID,
