		...
		<div class="row">
			<div class="col-sm-12">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item"><a href="{{ url('/') }}/admin/master/product/tax/" data-original-title="" title="">{{$PageTitle}}</a></li>
				...
				<div class="card-body " >
                    <div class="row">
                        <div class="col-sm-12 mt-20">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control  {{$Theme['input-size']}}" id="txtTaxName" value="<?php if($isEdit==true){ echo $EditData[0]->TaxName;} ?>">
                                <div class="errors" id="txtTaxName-err"></div>
                            </div>
                        </div>
                        <div class="col-sm-12 mt-20">
                            <div class="form-group">
                                <label class="txtPercentage"> Percentage <span class="required"> * </span></label>
                                <input type="number" step="0.01" class="form-control  {{$Theme['input-size']}}" id="txtPercentage" value="<?php if($isEdit==true){ echo $EditData[0]->TaxPercentage;} ?>">
                                <div class="errors" id="txtPercentage-err"></div>
                            </div>
                        </div>
                        <div class="col-sm-12 mt-20">
                            <div class="form-group">
                    ...
                    <div class="row mt-20">
                        <div class="col-sm-12 text-right">
                            @if($crud['view']==true)
                            <a href="{{url('/')}}/admin/master/product/tax" class="btn {{$Theme['button-size']}} btn-outline-dark mr-10" id="btnCancel">Back</a>
                            @endif
...
<script>
    $(document).ready(function(){
        const formValidation=()=>{
            ...
            if(TaxName==""){
                $('#txtTaxName-err').html('Tax Name is required.');status=false;
            }else if(TaxName.length<2){
                $('#txtTaxName-err').html('Tax Name must be greater than 2 characters');status=false;
            }else if(TaxName.length>100){
                $('#txtTaxName-err').html('Tax Name may not be greater than 100 characters');status=false;
            }
                        
            if(Percentage==""){
                $('#txtPercentage-err').html('The Percentage is required.');status=false;
            }else if($.isNumeric(Percentage)==false){
                $('#txtPercentage-err').html('The Percentage must be numeric.');status=false;
        ...
        $('#btnSave').click(function(){
            ...
            if(status){
                swal({
                    ...
                    swal.close();
                    btnLoading($('#btnSave'));
                    let postUrl=@if($isEdit==true) "{{url('/')}}/admin/master/product/tax/edit/{{$EditData[0]->TaxID}}"; @else "{{url('/')}}/admin/master/product/tax/create"; @endif
                        ...
                        success:function(response){
                            ...
                            if(response.status==true){
                                swal({
                                    ...
                                    @if($isEdit==true)
                                        window.location.replace("{{url('/')}}/admin/master/product/tax");
                                    @else
