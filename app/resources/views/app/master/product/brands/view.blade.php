				...
				<div class="card-header text-center">
					<div class="row">
						...
						<div class="col-12 col-sm-4 my-2 text-right">
							@if($crud['restore']==1)
								<a href="{{ url('/') }}/master/product/brands/trash" class="btn  btn-outline-dark {{$Theme['button-size']}} mr-10" type="button" > Trash </a>
							@endif
							@if($crud['add']==1)
								<a href="{{ url('/') }}/master/product/brands/create" class="btn  btn-outline-success btn-air-success {{$Theme['button-size']}}" type="button" >Create</a> <!-- full-right -->
							@endif
						</div>
					</div>
				</div>
    ...
    $(document).ready(function(){
        const LoadTable=async()=>{
			...
			$('#tblBrands').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url":"{{url('/')}}/master/product/brands/data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnEdit',function(){
			window.location.replace("{{url('/')}}/master/product/brands/edit/"+ $(this).attr('data-id'));
		});
		...
		$(document).on('click','.btnDelete',function(){
            ...
            function(){swal.close();
            	$.ajax({
            		type:"post",
                    url:"{{url('/')}}/master/product/brands/delete/"+ID,
