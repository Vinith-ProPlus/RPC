		...
		<div class="row">
			<div class="col-sm-12">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item"><a href="{{ url('/') }}/master/vendor/sub-category/" data-original-title="" title="">{{$PageTitle}}</a></li>
				...
				<div class="card-header text-center">
					<div class="row">
						...
						<div class="col-sm-4 my-2 text-right">
							@if($crud['view']==1)
								<a href="{{ url('/') }}/master/vendor/sub-category/" class="btn  btn-outline-dark {{$Theme['button-size']}}" type="button" > Back </a>
							@elseif($crud['add']==1)
								<a href="{{ url('/') }}/master/vendor/sub-category/create" class="btn  btn-outline-dark {{$Theme['button-size']}}" type="button" >Back</a> <!-- full-right -->
							@endif
						</div>
					</div>
				</div>
    ...
    $(document).ready(function(){
        const LoadTable=async()=>{
			...
			$('#tblVendorSubCategory').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url": "{{url('/')}}/master/vendor/sub-category/trash-data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnRestore',function(){
            ...
            function(){
            	...
            	$.ajax({
            		type:"post",
                    url:"{{url('/')}}/master/vendor/sub-category/restore/"+ID,
