		...
		<div class="row">
			<div class="col-sm-12">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item"><a href="{{url('/')}}/admin/master/vendor/vendor-type">{{$PageTitle}}</a></li>
			...
			<div class="card">
				...
				<div class="card-body " >
                    <div class="row mb-30  d-flex justify-content-center">
                        <div class="col-sm-6">
                            <input type="file" class="dropify imageScrop" data-aspect-ratio="{{$Settings['image-crop-ratio']['w']/$Settings['image-crop-ratio']['h']}}" data-remove="0" data-is-cover-image="1" id="txtSTImage" data-default-file="<?php if($isEdit==true){if($EditData[0]->STImage!=""){ echo url('/')."/".$EditData[0]->STImage;}}?>"  data-allowed-file-extensions="<?php echo implode(" ",$FileTypes['category']['Images']) ?>" >
                            <div class="errors" id="txtSTImage-err"></div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label class="txtVendorType">Vendor Type  <span class="required"> * </span></label>
                                <input type="text" class="form-control {{$Theme['input-size']}}" id="txtVendorType" value="<?php if($isEdit==true){ echo $EditData[0]->VendorType;} ?>">
                                <div class="errors" id="txtVendorType-err"></div>
                            </div>
                        </div>
                ...
                <div class="card-footer">
                    <div class="row">
                        <div class="col-sm-12 text-right">
                            @if($crud['view']==true)
                            <a href="{{url('/')}}/admin/master/vendor/vendor-type/" class="btn {{$Theme['button-size']}} btn-outline-dark mr-10" id="btnCancel">Back</a>
                            @endif
...
<script>
    ...
    $(document).ready(function(){
        ...
        const formValidation=async()=>{
            ...
            }else if(VendorType.length<2){
                $('#txtVendorType-err').html('Vendor Type must be greater than 2 characters');status=false;
            }else if(VendorType.length>100){
                $('#txtVendorType-err').html('Vendor Type may not be greater than 100 characters');status=false;
            }
        ...
        const GetData=async()=>{
            ...
            formData.append('VendorType',$('#txtVendorType').val());
            formData.append('ActiveStatus',$('#lstActiveStatus').val());
            formData.append('removeSTImage', $('#txtSTImage').attr('data-remove'));
            if(tmp.coverImage.uploadPath!=""){
                formData.append('STImage', JSON.stringify(tmp.coverImage));
            }
            return formData;
        }
        ...
        $('#btnSave').click(async function(){
            ...
            if(status){
                ...
                swal({
                    ...
                    swal.close();
                    btnLoading($('#btnSave'));
                    let postUrl= @if($isEdit) "{{url('/')}}/admin/master/vendor/vendor-type/edit/{{$EditData[0]->VendorTypeID}}"; @else "{{url('/')}}/admin/master/vendor/vendor-type/create"; @endif
                    $.ajax({
                        type:"post",
                        url:postUrl,
                        ...
                        success:function(response){
                            if(response.status==true){
                                swal({
                                    ...
                                    @if($isEdit==true)
                                        window.location.replace("{{url('/')}}/admin/master/vendor/vendor-type");
                                    @else
