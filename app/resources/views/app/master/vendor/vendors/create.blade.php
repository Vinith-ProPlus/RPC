		...
		<div class="row">
			<div class="col-sm-12">
				<ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item">@if($isEdit && !$isEditSL)Update @elseif($isEdit && $isEditSL) Update Service Locations @else Create @endif</li>
				</ol>
			</div>
		</div>
	</div>
		...
		<div class="col-12 col-sm-9 col-md-9 col-lg-9 mb-10">
            <div class="row">
                <div class="col-12 p-0">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control form-control-lg" id="txtVendorName" placeholder="Vendor Name" @if($isEdit && $isEditSL) disabled @endif value="<?php if($isEdit){ echo $data->VendorName;} ?>">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body p-0 woo-commerce-style">
                            <ul class="woo-commerce-style">
                                @if (!$isEditSL)
                                    <li class="general_options @if(!$isEditSL)active @endif"><a href="#general-tab"><span>General</span></a></li>
                                ...
                                @endif
                                <li class="service_location_options @if($isEditSL)active @endif"><a href="#service-location-tab"><span>Service Locations</span></a></li>
                            </ul>
                            @if (!$isEditSL)
                            ...
                            <div class="tab-contents" id="service-location-tab">
                                ...
                                <div class="row @if($isEdit && $data->ServiceBy == "Radius") @else d-none @endif divServiceBy my-2" id="divRadius">
                                    <div class="col-sm-12">
            ...
            <div class="row">
                <div class="col-12 text-right">
                    ...
                    <button type="button" class="btn btn-outline-success mr-10" id="btnSave">@if($isEdit && !$isEditSL)Update @elseif($isEdit && $isEditSL) Update Service Location @else Save @endif</button>
                </div>
            </div>
		</div>
        @if (!$isEditSL)
		...
		<div class="modal-content">
			...
			<div class="modal-body">
                <div class="row">
                    <div class="col-12">
                        <div class="form-group">
                            <label for="txtMSPName">Name</label>
                    ...
                    <div class="col-12 mt-10">
                        <div class="form-group">
                            <label for="txtMSPAddress">Address</label>
    ...
    $(document).ready(function(){
        ...
        const loadData=async(data)=>{
            ...
            if(data.length>0){
                ...
                for(let item of data.ServiceLocations.ServiceData){
                    let DistrictIDs = [];
                    if(data.ServiceLocations.ServiceBy == "District"){
                    ...
                    }else if (data.ServiceLocations.ServiceBy == "PostalCode") {
                        for (let district of item.Districts) {
        ...
        const getData=async()=>{
            ...
            let ServiceLocations={
                ServiceBy : $('.chkServiceBy:checked').data('value'),
                ServiceData : []
            };
            $('#vehicleAccordion .accordion-item').each(function(index){
                let t={};
        ...
        $(document).on('click','#btnSave',async function(){ 
            ...
            if(status){
                ...
                swal({
                    ...
                    btnLoading($('#btnSave'));
                    let postUrl= @if($isEdit && !$isEditSL) "{{url('/')}}/admin/master/vendor/manage-vendors/edit/{{$VendorID}}"; @elseif($isEdit && $isEditSL) "{{url('/')}}/admin/master/vendor/manage-vendors/edit/service-location/{{$VendorID}}"; @else "{{url('/')}}/admin/master/vendor/manage-vendors/create"; @endif
                    $.ajax({
                        type:"post",
                        url:postUrl,
                        headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
