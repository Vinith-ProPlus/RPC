		...
		<div class="row">
			<div class="col-sm-12">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item">Cancelled {{$PageTitle}}</li>
				</ol>
			</div>
		</div>
	</div>
			...
			<div class="card">
				<div class="card-header text-center">
					<div class="row">
						...
						<div class="col-sm-4 my-2"><h5>Cancelled {{$PageTitle}} </h5></div>
						<div class="col-sm-4 my-2 text-right">
							@if($crud['view']==1)
								<a href="{{ url('/') }}/admin/transaction/quote-enquiry/" class="btn  btn-outline-dark {{$Theme['button-size']}}" type="button" > Back </a>
							@elseif($crud['add']==1)
								<a href="{{ url('/') }}/admin/transaction/quote-enquiry/create" class="btn  btn-outline-dark {{$Theme['button-size']}}" type="button" >Back</a> <!-- full-right -->
							@endif
						</div>
					</div>
				</div>
    ...
    $(document).ready(function(){
        ...
        const LoadTable=async()=>{
			...
			$('#tblQuote').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url": "{{url('/')}}/admin/transaction/quote-enquiry/trash-data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnRestore',function(){
			...
			swal({
                title: "Are you sure?",
                text: "You want Restore this Quotation!",
                type: "warning",
            ...
            function(){
            	...
            	$.ajax({
            		type:"post",
                    url:"{{url('/')}}/admin/transaction/quote-enquiry/restore/"+ID,
        ...
        LoadTable();
		[[{"ProductID":"P2024-0000052"},{"ProductID":"P2024-0000053"},{"ProductID":"P2024-0000175"},{"ProductID":"P2024-0000180"}],[{"ProductID":"P2024-0000052"},{"ProductID":"P2024-0000053"},{"ProductID":"P2024-0000180"}],[{"ProductID":"P2024-0000180"}]]
