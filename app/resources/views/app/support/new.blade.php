<div class="row">
    ...
    <div class="col-sm-3">
        <div class="form-group">
            ...
            <select class="form-control mSelect2" id="lstSupportType">
...
<div class="row mt-20 mb-20">
    <div class="col-sm-12 text-right">
        ...
        <button id="{{$SaveButtonID}}" type="button" class="btn btn-outline-success btn-sm btn-success-air">Submit</button>
    </div>
</div>
<script>
    $(document).ready(function(){
        ...
        $('.Attachments').dropify();
        $(document).on('click','#btnCloseModal',function(){bootbox.hideAll();});
        const formValidation=async()=>{
            ...
            if (TicketFor == "Vendor") {
                UserID = $("#lstVendor").val();
                if (!UserID) {
                    $('#lstVendor-err').html('Vendor Name is required');
                    status = false;
                }
            } else {
            ...
            if(Subject==""){
                ...
                $('#txtSubject-err').html('Subject must be at least 3 characters');status=false;
            }else if(Subject.length>100){
                $('#txtSubject-err').html('Subject may not be greater than 100 characters');status=false;
            }
            if(SupportType==""){
                $('#lstSupportType-err').html('Support Type is required');status=false;
            }
            if(Description==""){
        ...
        $(document).on('click','#{{$SaveButtonID}}',async()=>{
            ...
            if((status==true)&&(blnprocess==false)){
                ...
                $.ajax({
					type:"post",
					url:"{{url('/')}}/admin/support/new-ticket/save",
					...
					error:function(e, x, settings, exception){blnProcessing=false;ajaxErrors(e, x, settings, exception);},
					complete: function(e, x, settings, exception){blnprocess=false;ajaxIndicatorStop();btnReset($('#btnSubmitSupport'));},
                        ...
                        if(response.status==true){
                            ...
                            if(response['errors']!=undefined){
								...
								$.each( response['errors'], function( KeyName, KeyValue ) {
									var key=KeyName;
									if(key=="UserID"){$('#lstCustomer-err').html(KeyValue);}
									if(key=="Subject"){$('#txtSubject-err').html(KeyValue);}
									if(key=="Priority"){$('#lstPriority-err').html(KeyValue);}
									if(key=="SupportType"){$('#lstSupportType-err').html(KeyValue);}
									if(key=="Description"){$('#txtDescription-err').html(KeyValue);}
									if(key=="Attachment1"){$('#imgAttachment1-err').html(KeyValue);}
									if(key=="Attachment2"){$('#imgAttachment2-err').html(KeyValue);}
									if(key=="Attachment3"){$('#imgAttachment3-err').html(KeyValue);}
									if(key=="Attachment4"){$('#imgAttachment4-err').html(KeyValue);}
								});
                            }
