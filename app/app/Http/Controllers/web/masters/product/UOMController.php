	...
	public function Delete(Request $req,$UomID){
		...
		if($this->general->isCrudAllow($this->CRUD,"delete")==true){
			...
			try{
				...
				$status=DB::table('tbl_uom')->where('UID',$UomID)->update(array("DFlag"=>1,"DeletedBy"=>$this->UserID,"DeletedOn"=>date("Y-m-d H:i:s")));
			}catch(Exception $e) {
				
			}
			if($status==true){
				DB::commit();
				$logData=array("Description"=>"Uom has been Deleted ","ModuleName"=>$this->ActiveMenuName,"Action"=>"Delete","ReferID"=>$UomID,"OldData"=>$OldData,"NewData"=>$NewData,"UserID"=>$this->UserID,"IP"=>$req->ip());
				logs::Store($logData);
